...
async function _fetchData(
  ...
  const data: Comic[] = $('li').map(parser($)).get();

  return data;
}

async function _fetchComics(
  params: Partial<RequestParameters>,
  options: Partial<FetchOptions>,
  parser: ParserFunction = Parsers.Issue
): Promise<Comic[]> {
  ...
  if (options?.sort && !Object.values(SortTypes).includes(options.sort)) {
    throw new RangeError(
      `The 'sort' option must be one of '${Object.values(SortTypes).join(
        "', '"
      )}'. Received '${options.sort}'.`
    );
  }
...
export function fetchCollection(
  userID: number,
  format: CollectionTypes = CollectionTypes.Issue,
  options?: Partial<FetchOptions>
): Promise<Comic[]> {
  const params = {
    list: 2,
  ...
  return _fetchComics(
    params,
    options,
    format === CollectionTypes.Issue ? Parsers.Issue : Parsers.Series
  );
}
...
export async function fetchUser(name: string): Promise<User> {
  ...
  const text = await axios.get(`${url}/pull-list`).then((res) => res.data);
